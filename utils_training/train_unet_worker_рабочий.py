...
EARLY_STOPPING_PATIENCE = 3                 # количество эпох без улучшения val_loss для остановки
...
def signal_handler(signum, frame):
    log(f"Получен сигнал {signum}. Сохраняем текущую модель...")
...
# ---------- Сохранение примеров валидации (отдельные файлы для меток и предсказаний) ----------
...
def main():
    ...
    for epoch in range(1, epochs + 1):
        ...
        if val_loader is not None:
            ...
            metrics = compute_metrics(all_preds, all_masks, num_classes, threshold=THRESHOLD_FACTOR)
        else:
            val_loss = None
            metrics = None
    ...
    if val_loader is not None:
        ...
        final_metrics = compute_metrics(all_preds, all_masks, num_classes, threshold=THRESHOLD_FACTOR)
